...
给定二叉树的层数和要删除的节点，画出二叉树，是一个比较麻烦的画图题。

### 思路

首先需要找规律得到树枝的长度，然后需要得到每一层节点的位置。对于树枝长度其实可以通过 [递推 @ailanxier](https://www.cnblogs.com/ailanxier/p/13556307.html) 得出，如下图所示.

![alt text](images/image-2.png)

所以第 i 层的树枝长 (第 i ~ i+1 层节点) 等于前 i-1 层树枝长度之和加节点数 (层数，i-1)。然后看每一层节点的位置，观察可知除了第一层外的其他层的同层相邻节点距离是一定的。所以确定每一层第一个节点的位置就可以推出其他节点的位置了。

- 对于水平位置，让第一层第一个节点水平位置为 1 。再次观察前面的图，可以发现第
i 层第一个节点的水平位置其实就是第 i 层的树枝长度 + 1 。
- 对于竖直位置，让根节点竖直位置为 1 。第 i 层竖直位置就其实就是第 i+1 层竖直位置 +
 第 i 层的树枝长度 + 1 。
- 确定完每一层第一个节点的位置后，接下来需要确定每一层后面节点的水平位置。可以发现除了最下面那层，其余层中相同层的节点间距是相同的，为 1 + 第 i 层的树枝长度的 2 倍。

接下来就可以绘制和删点了，这两个操作都是递归进行的。因为我们已经知道了每一层的树枝长度，所以我们可以从根节点开始建树，递归左右子树即可。

注意在最后打印的时候不要打印一个字符，应该 `"".join(ch[i][1:width+1])` 这样常数小很多。