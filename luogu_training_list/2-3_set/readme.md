...
![示例](images/image-2.png)

### 思路 1. 种类并查集 `@___new2zy___`

本题要求出现矛盾情况下的最小边权值，满足贪心性质（降序排序，如果两个人之间有矛盾，显然需要分开放，否则不是最小值）。所以需要先按矛盾值排序，然后每次选最大的关系，尝试把两个人分在不同组。如果成功则再取下个关系继续操作，如果失败，则输出当前矛盾值。

### 思路 2. 二分图 `@___new2zy___`
