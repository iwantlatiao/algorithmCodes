...
由于 `(x,y1,y2,1)` 和 `(x,y1,y2,-1)` 是成对出现的，所以不会出现父节点 `cnt>0` 子节点 `cnt=0` 时对子节点 `cnt-=1` 的情况。这种情况下，不下传信息，就使得当前区间被覆盖的次数，**跟其它节点无关**。也就是错误思路仅**去掉** `pushdown` 操作，就是正确思路。时间复杂度 $O(N\log N)$ 。
...
void build(int p,int l,int r) {  // build 没有 pushUp 操作了
    t[p]={l,r,0,0};
    if(l==r) return;
    int mid=l+r>>1;
...
[@一瞬流年丶涅槃](https://www.acwing.com/solution/content/4542/) 、[@Diaоsi](https://www.luogu.com.cn/article/wkxocuu4) 思路：

由于矩形的大小固定，所以矩形可以由任意一个顶点唯一确定。不妨考虑将矩形的右上顶点放在什么位置圈住的星星 `(x, y, c)` 亮度之和最大。

可以对每一个星星建立一个固定大小的矩形，矩形的左下角为 `(x,y)` 。因为边界上的星星不算，不妨将边框长宽都减少 `0.5` ，所以右上角表示为 `(x+w-1,y+h-1)` 。于是我们可以将每个星星都扩展成一个左下顶点在星星上的矩形，这时我们注意到，若两个矩形之间有交集，他们便可以放在同一个窗户中（右上顶点的位置在交集中）。

此时我们可以将问题转化为：平面上有若干个矩形，每个矩形都带有一个权值，求在哪个坐标上权值的总和最大。

接下来我们就可以使用扫描线来解决这个问题了，若当前星星的亮度值为 `l` 则矩形的入边的权值设为 `l` ，出边为 `−l` ，此时我们只要求扫描线上的**区间最大值**即可得出答案，区间查询可以使用懒标记的方式实现。

还有一点要注意的是在根据横坐标排序的时候，如果**横坐标相同**也就是线段重合的时候。注意**先加上新的再减去旧的**，否则重合的情况就没被算到。

```c++
// 每个左右边界保存为两个四元组
add(x, y, y+h-1, c); add(x+w, y, y+h-1, -c);
```