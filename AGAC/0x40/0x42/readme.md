...
题意：给一个长度为 1e6 的数列和长度为 1e6 的询问，每个询问给一个区间，要求回答区间内不同数字的个数。

[@dlhham](https://www.luogu.com.cn/article/boho8rhn) 、[@叶小枫](https://www.luogu.com.cn/article/iilru8ad) 思路：离线 + 树状数组维护。

首先应考虑到这样一个结论：对于若干个询问的区间 `[l,r]` ，如果他们的 `r` 都相等的话，那么数列中出现的同一个数字，只需要关注出现在最右边的那一个。例如对于 `1 3 4 5 1` ，对于 `r=5` 的所有询问，第一个位置上的 `1` 就没有意义，可以用右边的 `1` 替代。（当然也可以关注左端点，不过得反着建树。重点在于维护最靠近端点的不同类数字。）

既然只关注最右边的数字，那么就需要依次从左向右地处理和修改整个数组，这样才能保证我们对于前一个区间的修改不会影响到后面区间的询问。当然还有一点，在关注了最右边数字之后，要忽略区间内其他同类的数字。

所以树状数组维护的是**有多少不同类的数字**。因为由于我们之前的修改操作，所有同类的数字已经被简化到只剩最靠右边那个，所以可以保证这种做法的正确性。

综上所述，可以对所有询问按照右端点排序。对于每次询问，将树状数组维护到右端点。维护包含两个操作：

1. 如果某元素 `i` 在之前已经出现过（位置记为 `vis[i]` ），那么将其以前最右端位置的前缀和减一，相当于忽略之前的位置。再在现在的位置把前缀和加一，并更新 `vis[i]` 到当前位置。
2. 如果某元素 `i` 在之前没有出现，那么直接在现在的位置把前缀和加一，并更新 `vis[i]` 。

```c++
// @叶小枫
int main(){
    /* ... 读入 ... */
	sort(q+1,q+m+1,cmp);  // q: l, r, id; 按照 r 排序
	
	int pow=1;  // 树状数组未维护到的最左端点（已维护最右端点 + 1）
	for(rint i=1;i<=m;++i){
		for(rint j=pow;j<=q[i].r;++j){
			if(vis[a[j]]) modify(vis[a[j]],-1);
			modify(j,1);
			vis[a[j]]=j;  // 该种贝壳在已维护区间中最后出现的位置
		}
		pow=q[i].r+1;
		ans[q[i].id]=query(q[i].r)-query(q[i].l-1);  // 按照原序输出
	}
